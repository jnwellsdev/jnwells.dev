<template lang="pug">
main 
	template(v-if='store.allowed || store25.allowed')
		header
			h1(@click='store.allowed ? router.push("/2025") : router.push("/")') Jesse Wells
			p(v-html='store.isDesign ? store.data.design.recentWork : store.data.dev.recentWork')
		section.work
			template(v-for='card in store.data.work.slice(0,4)')
			...
			template(v-for='card in store.data.work.slice(4,5)')
				Card.ad(:pt='pCardAd' @click='handleLink(card)')
					template(#content)
						video(width='972' height='248' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
			template(v-for='card in store.data.work.slice(5,9)')
				Card.desk(:pt='pCard' @click='handleLink(card)')
					template(#content)
						video(width='1184' height='660' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
			template(v-for='card in store.data.work.slice(9,12)')
				Card.mob(:pt='pCardSm' @click='handleLink(card)')
					template(#content)
						video(width='548' height='1138' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
			template(v-for='card in store.data.work.slice(12,14)')
				Card.desk(:pt='pCard' @click='handleLink(card)')
					template(#content)
						video(width='1184' height='660' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
			template(v-for='card in store.data.work.slice(14,15)')
				Card.ad(:pt='pCardAd' @click='handleLink(card)')
					template(#content)
						video(width='972' height='248' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
			template(v-for='card in store.data.work.slice(15,19)')
				Card.desk(:pt='pCard' @click='handleLink(card)')
					template(#content)
						video(width='1184' height='660' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
			template(v-for='card in store.data.work.slice(21,24)')
				Card.mob(:pt='pCardSm' @click='handleLink(card)')
					template(#content)
						video(width='548' height='1138' muted autoplay loop style='width: 100%; height: 100%;border-radius: 10px;')
							source(:src='`/video/demo${card.id}.webm`' type='video/webm')
